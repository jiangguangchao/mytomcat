                ...
                try {
                    if (key == null) {
                        ...
                        if (socketWrapper != null) {
                            ...
                            //jgc备注
                            //这里是将SelectionKey 的interestOps 重新设置为1。这里是为了复用socket连接
                            //通常一个页面可能会发起很多请求，这些请求可以复用socket。比如一个页面包含了50个请求,
                            //当前页面会和服务器（tomcat）间最多建立6个socket连接（第一次启动tomcat后发起的请求，好像最多可以建立7个socket连接），
                            //这个socket数量的限制应该是浏览器做出的。注意，这是的数量限制是指同一个页面发起的请求所建立的socket连接。不同页面
                            //不知道能否复用socket，还没测试。通常一个socket不会保留很久，如果某个socket连接超过1秒（调试时感觉应该是1秒，具体数值不清楚）
                            //没有被请求使用，则会被丢弃。
                            //通常代码执行到这里，也就意味着上一个请求已经处理完毕。此时需要将key的interestOps设置为1，以便接受后续的请求。
                            //因为当socket收到一个请求后，需要先将key的interestOps设置为0,然后处理这个收到的请求。因为是0，所以处理当前请求的过程中，
                            //当前的socket不会再接受其他请求。请求处理完毕后。又将interestOps设置为1。这样就可以继续接受其他请求了。
                            //所以就起到了socket的复用效果。
        ...
        public NioSocketWrapper cancelledKey(SelectionKey key) {
            ...
            try {
                ...
                if (ka != null) {
                    try {
                        //jgctodo
                        System.out.println("开始关闭socket：" + ka.getSocket());
...
//                set.forEach(sk -> {
//                    System.out.println("selector 的key：" +sk.attachment());
//                });
...
//                    System.out.println();
                    ...
                    if (sk.interestOps() == 1) {
                        eventName = "读";
                    } else if (sk.interestOps() == 4) {
                        eventName = "写";
                    }
                    System.out.println("Pooler捕获到selector中的事件，该事件为[" + eventName + "]");
                ...
                //jgctodo
//                System.out.println("Process timeouts........ keyCount: " + keyCount + ", hasEvents: " + hasEvents
//                    + "系统时间：" + System.currentTimeMillis());
        ...
        //jgc备注
        //收到一个请求，开始进行处理。处理之前会先将SelectionKey对象的interestOps设置为0，
        // 防止SelectionKey对象对应的socket连接继续接受其他请求。在当前请求处理完毕后，
        // 会再次将interestOps设置为1，继续接受其他请求。起到复用socket连接的效果，
        // 也就是同一个socket连接可以处理多个请求（通常是指一个页面的多个请求）。
                ...
                if (close) {
                    ...
                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {
                            ...
                            //jgctodo
//                            System.out.println("sk.readyOps(): " + sk.readyOps() + "  @" + Integer.toHexString(sk.hashCode()));

                            //jgc备注
                            //这里会将sk的interestOps设置为0，防止sk对应的socket继续接受其他请求。
                            unreg(sk, attachment, sk.readyOps());

                            //jgctodo
//                            System.out.println("sk的 interestOps是否被置为0 : " + sk.interestOps() + "  @" + Integer.toHexString(sk.hashCode())
//                                + "， 开始处理收到的请求：" + System.currentTimeMillis());
                    ...
                    try {
                        ...
                        if ( ka == null ) {
                            ...
                            if ((ka.interestOps() & SelectionKey.OP_READ) == SelectionKey.OP_READ) {
                                ...
                                //jgctodo
//                                System.out.println("设备isTimedOut的值，设置后：" + isTimedOut + ",  timeout: " + timeout
//                                    + ", 当前系统时间：" + System.currentTimeMillis());
    ...
    protected class SocketProcessor extends SocketProcessorBase<NioChannel> {
        ...
        protected void doRun() {
            ...
            //jgctodo
            System.out.println("线程池Executor中的SocketProcessor对象开始执行doRun方法处理socket请求");
                ...
                try {
                    if (key != null) {
                        if (socket.isHandshakeComplete()) {
                            ...
                            //jgctodo
//                            System.out.println("socket.isHandshakeComplete()？？？？？？？？？？？？？？？");
                ...
                //jgctodo
//                System.out.println("handshake: " + handshake + ", event:" + event);
                ...
                if (handshake == 0) {
                    ...
                    if (event == null) {
                        ...
                        //jgctodo
                        System.out.println("开始获取handler对象处理请求");
