...
public class Http11Processor extends AbstractProcessor {
    ...
    public SocketState service(SocketWrapperBase<?> socketWrapper)
        ...
        while (!getErrorState().isError() && keepAlive && !isAsync() && upgradeToken == null &&
            ...
            try {
                ...
                if (endpoint.isPaused()) {
                    ...
                    System.out.println("标记点1111111111111");
                    ...
                    System.out.println("标记点2222");
                    if (!http09 && !inputBuffer.parseHeaders()) {
                        System.out.println("标记点33");
        ...
        if (getErrorState().isError() || (endpoint.isPaused() && !isAsync())) {
            System.out.println("返回标记点1");
            return SocketState.CLOSED;
        } else if (isAsync()) {
            System.out.println("返回标记点2");
            return SocketState.LONG;
        } else if (isUpgrade()) {
            System.out.println("返回标记点3");
            return SocketState.UPGRADING;
        } else {
            if (sendfileState == SendfileState.PENDING) {
                System.out.println("返回标记点4");
                return SocketState.SENDFILE;
            } else {
                if (openSocket) {
                    if (readComplete) {
                        System.out.println("返回标记点5");
                        return SocketState.OPEN;
                    } else {
                        System.out.println("返回标记点6");
                        return SocketState.LONG;
                    }
                } else {
                    System.out.println("返回标记点7");
                    return SocketState.CLOSED;
                }
            }
        }
